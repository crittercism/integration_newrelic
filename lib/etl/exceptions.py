...
class ExceptionETL(ErrorsByVersionProcessor):
    ...
    def __init__(self, app_id, lookback_minutes, new_relic_account, new_relic_app_id):
        super(ExceptionETL, self).__init__(
            app_id,
            lookback_minutes,
            new_relic_account,
            new_relic_app_id,
    ...
    def get_errors_with_details(self, lookback_minutes):
        exception_summaries = self._c_client.app_exception_summaries(
            self._app_id,
            lookback_timedelta=timedelta(minutes=lookback_minutes)
        )
        exceptions = []
        for summary in exception_summaries['data']:
            exceptions.append(self._c_client.exception_details(
                self._app_id,
                summary[u'hash'])
            )
